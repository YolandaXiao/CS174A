...
// *********** Waterdrop ***********   ( With lattitude / longitude divisions; this creates singularities in the mesh at the top and bottom.  An alternative is at the bottom of this file. )

Make_Shape_Subclass( "Waterdrop", Shape );
  Waterdrop.prototype.populate = function( recipient, rows, columns, points_transform = mat4() ) 
    {  
      var circle_points = [];
      for( var i = 0; i <= rows; i++ )
      {
        circle_points.push( vec3( Math.cos( i/rows * Math.PI - Math.PI/2 )*(1-Math.sin(i/rows * Math.PI - Math.PI/2)), 0, 1.5*(Math.sin( i/rows * Math.PI - Math.PI/2 ) - 1) ) );
      }
      Patch.prototype.populate( recipient, rows, columns, 360, circle_points, false, points_transform );     
    }  

// *********** HALF SPHERE ***********   ( With lattitude / longitude divisions; this creates singularities in the mesh at the top and bottom.  An alternative is at the bottom of this file. )

Make_Shape_Subclass( "Hemisphere", Shape );
  Hemisphere.prototype.populate = function( recipient, rows, columns, points_transform = mat4() ) 
    {  
      var circle_points = [];
      for( var i = 0; i <= rows; i++ )   circle_points.push( vec3( Math.cos( i/rows * Math.PI - Math.PI/4 ), 0, Math.sin( i/rows * Math.PI - Math.PI/4 ) ) );
