...
// Make_Shape_Subclass( "Hex", Shape );
//   Tetrahedron.prototype.populate = function( using_flat_shading )   // Takes a boolean argument 
//   {
//     var a = 1/2;    
//     var b = Math.sqrt(3)/2;  
//     var c = 1/(Math.sqrt(3)*3);
//     var d = Math.sqrt(2);   
    
//     if( !using_flat_shading )
//     {
//       // Method 1:  A tetrahedron with shared vertices.  Compact, performs better, but can't produce flat shading or discontinuous seams in textures.
//         this.positions     .push( vec3(1,0,0),     vec3(-1,0,0),    vec3(a,b,0),      vec3(a,-b,0),      vec3(-a,b,0),      vec3(-a,-b,0),      vec3(0,0,Math.sqrt(6)), vec3(0,0,0));
//         this.normals       .push( vec3(0,-4c,c*d), vec3(0,4c,c*d),  vec3(2c,2/3,c*d), vec3(-2c,2/3,c*d), vec3(2c,-2/3,c*d), vec3(-2c,-2/3,c*d), vec3(0,0,-1),           vec3(0,0,1));
//         this.texture_coords.push( vec2(0,0),      vec2(0,1),   vec2(1,0),   vec2(1,1)   );
//         this.indices.push( 0, 1, 2,   0, 2, 3,   0, 3, 4,    0, 4, 5,    0, 5, 6,   0, 6, 1,  7, 1, 2,   7, 2, 3,   7, 3, 4,    7, 4, 5,    7, 5, 6,   7, 6, 1);          // Vertices are shared multiple times with this method.
//     }   
//     else
//     {
//       // Method 2:  A tetrahedron with four independent triangles.  
...
Make_Shape_Subclass( "Tri", Shape );
    Tri.prototype.populate = function()
    {
        // for( var i = 0; i < 2; i++ )     // A loop to automatically generate the triangles.
        // {
          //first part
            var spin = rotation( 0, 0, 1, 0 );             // Rotate around a few degrees in XZ plane to place each new point.
            var newPoint = mult_vec( spin, vec4( 1, 0, 0, 1 ) );            // Apply that XZ rotation matrix to point (1,0,0) of the base triangle.
            this.positions.push( vec3( newPoint[0], 0, newPoint[2] ) );     // Store this XZ position.  This is point 1.
            this.positions.push( vec3( 0, 1, 0 ) );     // Store it again but with higher y coord:  This is point 2.
            this.positions.push( vec3( 0, 0, 0 ) );                         // All triangles touch this location.  This is point 3.
            ...
            this.texture_coords.push( vec2( 0, 1 ) );                       // Repeat the same arbitrary texture coords for each fan blade.
            this.texture_coords.push( vec2( 1, 0 ) );                       
            this.indices.push ( 0 );     this.indices.push ( 1 );        this.indices.push ( 2 ); // Procedurally connect the new


            //second part
            var newspin = rotation(  90, 0, 1, 0 );             // Rotate around a few degrees in XZ plane to place each new point.
            var newnewPoint = mult_vec( newspin, vec4( 1, 0, 0, 1 ) );            // Apply that XZ rotation matrix to point (1,0,0) of the base triangle.
            this.positions.push( vec3( newnewPoint[0], 0, newnewPoint[2] ) );     // Store this XZ position.  This is point 1.
            this.positions.push( vec3( 0, 1, 0  ) );     // Store it again but with higher y coord:  This is point 2.
            this.positions.push( vec3( 0, 0, 0 ) );                         // All triangles touch this location.  This is point 3.
            
            var newnewNormal = mult_vec( newspin, vec3( 0, 0, 1 ) );              // Rotate our base triangle's normal (0,0,1) to get the new one.  Careful! 
            this.normals.push( vec3( newnewNormal ) );                         // Normal vectors are not points; their perpendicularity constraint gives them 
            this.normals.push( vec3( newnewNormal ) );                         // a mathematical quirk that when applying matrices you have to apply the 
            this.normals.push( vec3( newnewNormal ) );                         // transposed inverse of that matrix instead.  But right now we've got a pure 
                                                                            // rotation matrix, where the inverse and transpose operations cancel out.
            this.texture_coords.push( vec2( 0, 0 ) );                       
            this.texture_coords.push( vec2( 0, 1 ) );                       // Repeat the same arbitrary texture coords for each fan blade.
            this.texture_coords.push( vec2( 1, 0 ) );                       
            this.indices.push ( 3 );     this.indices.push ( 4 );        this.indices.push (5 ); // Procedurally connect the new
